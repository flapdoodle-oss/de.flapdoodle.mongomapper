...
import de.flapdoodle.mongomapper.ListMapper;
...
public class RootMapper extends AbstractAttributeMapper<Root> {
    ...
    private static final ObjectAsAttributeMapper<ImmutableList<? extends Sub>, ListMapper<Sub,SubMapper>> sub=new ObjectAsAttributeMapper<ImmutableList<? extends Sub>, ListMapper<Sub,SubMapper>>("sub", new ListMapper<Sub,SubMapper>(SubMapper.INSTANCE));
...
//    }
    ...
    protected Root asObject(AttributeValueMap attributes) {
        return new Root(attributes.get(foo), attributes.get(bar),attributes.get(created), attributes.get(sub));
    }
